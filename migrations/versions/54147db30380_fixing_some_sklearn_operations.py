...
def _insert_operation_form_field():
    ...
    data = [
        (4113, 'output_distribution', 'TEXT', 0, 4, 'uniform', 'dropdown', None,
         '[{"key": \"quantile\", \"value\": \"quantile\"}, '
         '{\"key\": \"uniform\", \"value\": \"uniform\"}, '
         '{\"key\": \"kmeans\", \"value\": \"kmeans\"}]', 'EXECUTION', 4014, None),
...
def _undo_operation_form_field():
    ...
    columns = ('id', 'name', 'type', 'required', 'order', 'default',
               'suggested_widget', 'values_url', 'values', 'scope', 'form_id', 'enable_conditions')
    data = [
        (4088, 'feature', 'TEXT', 0, 3, None, 'attribute-selector', None, '{\"multiple\": false}', 'EXECUTION', 4017,
         None),
        (4073, 'output_distribution', 'TEXT', 0, 4, 'uniform', 'dropdown', None,
         '[{"key": \"normal\", \"value\": \"normal\"}, '
         '{\"key\": \"uniform\", \"value\": \"uniform\"}]', 'EXECUTION', 4014, None),
        (4074, 'seed', 'INTEGER', 0, 5, None, 'integer', None, None, 'EXECUTION', 4014, None),
...
def _insert_operation_form_field_translation():
    ...
    data = [
        (4113, 'en', 'Strategy', 'Strategy used to define the widths of the bins.'),
        (4113, 'pt', 'Estratégia', 'Estratégia utilizada para definir o range dos bins.'),
    ]
    rows = [dict(list(zip(columns, row))) for row in data]
    op.bulk_insert(tb, rows)
...
all_commands = [
    ...
    (_insert_operation_form_field, "DELETE FROM operation_form_field WHERE id = 4113"),
    (_insert_operation_form_field_translation, 'DELETE FROM operation_form_field_translation WHERE id = 4113'),
